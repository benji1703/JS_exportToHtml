...
function generateHtml() {
  ...
  // Get the root
  var root = xmlDoc.getElementById('root');
  var rootchild = root.childNodes[0];
  var hscene = xmlDoc.getElementsByTagName('a-scene')[0];

root.replaceChild(hscene, rootchild);
  ...
  scene
    .parentNode
    .replaceChild(sceneTemp, scene);

  var output = xmlToString(xmlDoc).replace('<a-scene-temp></a-scene-temp>', getClipboardRepresentation(sceneEl));
...
// Returns a copy of the DOM hierarchy prepared for serialization. The process
// optimises component representation to avoid values coming from primitive
// attributes, mixins and defaults.
...
// primitive attributes, mixins, injected default components or schema
// defaults.
...
// Computes the value for a component coming from primitive attributes, mixins,
// primitive defaults, a-frame default components and schema defaults. In this
// specific order. In other words, it is the value of the component if the
// author would have not overridden it explicitly.
function getImplicitValue(component, source) {
  var isInherited = false;
  var value = (isSingleProperty(component.schema) ?
    _single :
    _multi)();
...
function isSingleProperty(schema) {
  return AFRAME
    .schema
    .isSingleProperty(schema);
}

// Gets the value for a component or component's property coming from mixins of
// an element. If the component or component's property is not provided by
// mixins, the functions will return `undefined`.
function getMixedValue(component, propertyName, source) {
  var value;
  var reversedMixins = source
    .mixinEls
    .reverse();
...
// The string representation of data according to the passed component's schema.
function stringifyComponentValue(schema, data) {
  ...
  return (isSingleProperty(schema) ?
    _single :
    _multi)();
  ...
  function _multi() {
    ...
    return AFRAME
      .utils
      .styleParser
      .stringify(propertyBag);
  }
}

// Gets the value for the component's property coming from a primitive
// attribute. Primitives have mappings from attributes to component's
// properties. The function looks for a present attribute in the source element
// which maps to the specified component's property.
